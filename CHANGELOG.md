...
> 本节记录当前开发工作区中已完成验证、尚待拆分发布的更新。

### 🎙️ 新模块 — 语音合成

- **语音合成工作台**：提供 CosyVoice 文本合成、音色试听、音色命名和效果预设能力。
- **受控音色能力**：复刻、设计、管理音色改为功能级权限；本地保存试听音频和已删除音色标记，避免上游列表延迟造成误展示。
- **音频链路完善**：新增上传校验、对象存储探测及可读下载文件名处理。

### 📊 中价课课程看板

- 下线旧课程流转与课中数据占位入口，新增按期次与 SKU 分析的课程看板。
- 服务端支持受控数据刷新与每日缓存；前端提供趋势、结构及转化指标对比。

### 🖼️ 图片与视频工具增强

- **图片去背景**：新增 GIF 逐帧去背、GIF 预览、单文件下载与 ZIP 打包；服务端限制最多 30 帧以控制处理成本。
- **视频抽帧**：支持将抽取帧生成 GIF，并补充预览面板。
- **图像压缩与 Base64 转换**：优化图片预览、导出及交互体验。
- **特效生成器**：完善粒子预设、参数控制、画布预览和 GIF 录制。

### ✍️ 文案与效率工具

- 新增「正则规则工作台」：支持 AI 协作、JSON 导入、规则编辑、测试和 JavaScript 导出。
- 新增「图标库」：支持 Iconify 图标搜索和多格式复制。

### 🧹 模块重组与安全稳定性

- 下线素材生成工作台、旧 TTS 工作室、逐字稿审查/生成和案例设计等退役模块，并同步清理权限数据。
- AI 新闻改为 RSS 优先的容错数据源；默认缓存 60 分钟。
- 固定 Node `v20.19.0` 与 pnpm `10.33.0` 基线，新增 `pnpm run doctor` 诊断运行时、服务、新闻、CLI 与 Git 状态。
- 补齐登录令牌续期的响应码配置和默认兜底，避免环境变量漏配时跳过静默刷新。
